function [c, a, obj, exitflag, output] = identify_single_kernel_fixed_order(t, M, alpha, opts, varargin)

% Use MEX routines?
UseMEXRoutines = true;
if(~isempty(opts) && isfield(opts, 'UseMEXRoutines')), UseMEXRoutines = opts.UseMEXRoutines; end

% Use long or double in kernel computations?
UseLongKernelComputations = false;
if(~isempty(opts) && isfield(opts, 'UseLongKernelComputations')), UseLongKernelComputations = opts.UseLongKernelComputations; end
